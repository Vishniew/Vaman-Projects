...
int main() {
    ...
    while (true) {
        // 1. Update the display with the current value
        int32_t mask = bits[val] << FIRST_GPIO;
        gpio_set_mask(mask);

        // 2. Check if button is pressed (LOW)
        if (!gpio_get(BUTTON_GPIO)) {
            // Increment the value
            if (val == 9) {
                val = 0;
            ...
            // 3. WAIT here until the button is released
            // This prevents the counter from racing through numbers
            while(!gpio_get(BUTTON_GPIO)) {
                sleep_ms(10); 
            }
            
            // 4. Clear the old mask before the next loop updates it
            gpio_clr_mask(mask);
        ...
        // No "else" block here, so if no button is pressed, 
        // the code just keeps showing the current 'val'.
    }
}